  ...
  switch (cmd_code)
    {
      ...
      case 1012:  // void mpfr_set_zero (mpfr_t x, int sign)
      {
        ...
        for (size_t i = 0; i < length (&idx); i++)
          fcn (&mpfr_data[(idx.start - 1) + i], (int) sign_ptr[i * sign_stride]);
        return;
      }
      ...
      case 1166:  // void mpfr_nexttoward (mpfr_t x, mpfr_t y)
      {
        ...
        if (cmd_code == 1013)
          for (size_t i = 0; i < length (&x); i++)
            mpfr_swap (&mpfr_data[x.start - 1 + i], &mpfr_data[y.start - 1 + i]);
        else
          for (size_t i = 0; i < length (&x); i++)
            mpfr_nexttoward (&mpfr_data[x.start - 1 + i], &mpfr_data[y.start - 1 + i]);
        return;
      }
        ...
        for (size_t i = 0; i < length (&rop); i++)
          {
            ...
            ret_ptr[i * ret_stride] = (double) mpfr_init_set (
              &mpfr_data[(rop.start - 1) + i], &mpfr_data[(op.start - 1) + i], rnd);
          }
        return;
      }
      ...
      case 1017:  // double mpfr_get_d (mpfr_t op, mpfr_rnd_t rnd)
      {
        ...
        for (size_t i = 0; i < length (&op); i++)
          ret_ptr[i] = (double) mpfr_get_d (&mpfr_data[(op.start - 1) + i], rnd);
        return;
      }
      ...
      case 1019:  // int mpfr_frexp (mpfr_exp_t *exp, mpfr_t y, mpfr_t x, mpfr_rnd_t rnd)
      {
        ...
        for (size_t i = 0; i < length (&y); i++)
          {
            mpfr_exp_t exp = 0;
            ret_ptr[i] = (double) mpfr_frexp (&exp, &mpfr_data[(y.start - 1) + i],
                                              &mpfr_data[(x.start - 1) + i], rnd);
            exp_ptr[i] = (double) exp;
          }
        return;
      }
